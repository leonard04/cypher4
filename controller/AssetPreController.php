...
//        echo '<pre>'.print_r($request->id_item,1).'</pre>';
        ...
        $fr_num = $this->nextDocNumber('FR',$request_date);
//        dd($fr_num);
    ...
    // PEV
    function indexPev(){
        $pev = Asset_pre::where('company_id', Session::get('company_id'))
            ->where('pev_num', '!=', null)
            ->get();
        ...
        $pro = Marketing_project::where('company_id', Session::get('company_id'))->get();
        $pro_name = array();
...
//            dd($newpo);
            ...
            for ($x = 0; $x < count($newpo); $x++){
                $po_num = Asset_po::where('created_at', 'like', ''.date('Y')."-%")->first();
                if (!empty($po_num)) {
                ...
                $po->po_num = $num."/".strtoupper(Session::get('company_tag'))."/PO/".$arrRomawi[date('n')]."/".date('y');
                ...
                $po->company_id = Session::get('company_id');
                $po->save();
                foreach ($newpo[$x] as $idpo => $itempo){
