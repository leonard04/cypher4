                ...
                <table class="table table-bordered table-hover display font-size-sm" style="margin-top: 13px !important; width: 100%;">
                    ...
                    @foreach($main as $key => $value)
                        ...
                        <tr @if($diff < 30) class="bg-danger text-white" @endif>
                            ...
                            <td class="text-center">
                                <a href="{{route('mcu.logs',['id' => $value->id])}}" title="Log Document" class="btn btn-sm btn-primary btn-icon btn-xs"><i class="fa fa-search"></i></a>
                            </td>
                            <td width="10%" class="text-center">

                                <a href="{{route('mcu.delete',['id' => $value->id])}}" title="Delete" class="btn btn-sm @if($diff < 30) btn-secondary @else btn-danger @endif btn-icon btn-xs" onclick="return confirm('Delete MCU?')"><i class="fa fa-trash"></i></a>
                            </td>
                        </tr>
                    @endforeach
                    </tbody>
