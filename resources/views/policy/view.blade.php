    ...
    <div class="card card-custom gutter-b">
        <div class="card-header">
            <div class="card-title">
                <a href="{{route('policy.index')}}" title="Policy" class="btn btn-success"><i class="fa fa-backspace"></i>&nbsp;Back</a>
            </div>
            <div class="card-toolbar">
                <div class="btn-group" role="group" aria-label="Basic example">

                    <a href='javascript:framePrint("print_frame")' target='_self' class='btn btn-success'><i class='fa fa-print'></i> Print</a>&nbsp;&nbsp;&nbsp;
                    <iframe src='' width='0' height='0' frameborder='0' name='print_frame'></iframe>
                </div>
                <!--end::Button-->
            </div>
        </div>
        <div class="card-body">
            ...
            <div class="row">
                <div class="col-md-6">
                    <center> <h4>INDONESIAN</h4> </center>
                    <div class="col-md-6 text-center">
                        <p>{{strip_tags($detail->content)}}</p>
                    </div>
                </div>
                <div class="col-md-6">
                    <center> <h4>ENGLISH</h4> </center>
                    <div class="col-md-6 text-center">
                        <p>{{strip_tags($detail->content_eng)}}</p>
                    </div>
                </div>
            </div>
            <br>
                        ...
                        @if($type != null)
                            @if($type == 'ack')
                                @if($detail->acknowledge_by == null)
                                    <form action="{{route('policy.detail.viewappr.submit')}}" method="POST">
                                        @csrf
