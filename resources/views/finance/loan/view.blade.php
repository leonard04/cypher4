...
@section('content')
    <div class="card card-custom gutter-b">
        ...
        <div class="card-body">
            ...
            <div class="m-5">
                <form action="{{URL::route('loan.save_plan')}}" method="post">
                    @csrf
                    <h3>Loan Detail</h3>
                    <hr>
                    <table class="table display">
                        ...
                        ?>
                            @for($i=1; $i <= $loan->period; $i++)
                                <?php
                                    $rate = ($loan->value * (($loan->bunga/100) / 365 * date('t', strtotime($perbayar[$i]))));
                                    if ($i > $loan->cicil_start){
                                        $cicilan = round($loan->value / ($loan->period - $loan->cicil_start), 2);
                                        $read = "";
                                    } else {
                                        $cicilan = 0;
                                        $read = "readonly";
                                ...
                                <tr>
                                    ...
                                    <td align="center">
                                        <input type="text" id="cicilan[{{$i}}]" onchange="calc_1({{$i}})" {{$read}} class="form-control kt_inputmask_6" name="installment[{{$i}}]" value="{{$cicilan}}">
                                    </td>
                                    <td>
                                        <input type="text" id="percent[{{$i}}]" onchange="calc_3({{$i}})" class="form-control kt_inputmask_6" name="rate[{{$i}}]" value="{{$loan->bunga}}">
                                    </td>
                                    <td align="right">
                                        <input type="text" class="form-control kt_inputmask_6" id="bunga[{{$i}}]" name="interest[{{$i}}]" readonly value="{{number_format($rate, 2)}}">
                                    </td>
                                    <td align="right">
                                        <input type="text" class="form-control kt_inputmask_6" id="total_bayar[{{$i}}]" readonly value="{{number_format($total_amount, 2)}}">
                                    </td>
                                    <td align="center">
                                        planned
                                    </td>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-2">
                            ...
                            @if(count($loan_item) > 0)
                                <a href="{{URL::route('loan.edit_plan', $loan->id)}}" class="btn btn-primary btn-xs"><i class="fa fa-pencil-alt"></i> Edit Plan</a>
                            @else
