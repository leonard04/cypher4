                    ...
                    @foreach($tre_his as $key => $value)
                        <tr>
                            ...
                            <td align="center">
                                ...
                                <?php /** @var TYPE_NAME $balance */
                                /** @var TYPE_NAME $value */
                                $balance -= $value->IDR ?>
                            </td>
                            <td align="center">
                                @if(isset($sp[$value->sp_date]))
                                    <label for="">
                                        @php
                                            /** @var TYPE_NAME $value */
                                            /** @var TYPE_NAME $sp */
                                            $num = explode("/", $sp[$value->sp_date]->num);
                                            if (intval($num[0]) % 2 == 0){
                                                $bg = "label-primary";
                                            } else {
                                                $bg = "label-warning";
                                        ...
                                        @endphp
                                        <a href="{{route('treasury.viewsp', $sp[$value->sp_date]->id)}}"><span class="label label-inline {{$bg}}">{{$sp[$value->sp_date]->num}}</span></a>
                                    </label>
                                @endif
                                @if($value->sp_app != 1)
                                    <label class="checkbox checkbox-outline-2x checkbox-outline checkbox-success mx-auto justify-content-center check-sp">
                                        <input type="checkbox" name="sp[]" onchange="addToSP(this)" value="{{$value->id." ".$value->date_input}}"/>
                                        <span></span>
                                    </label>
                                @endif
                            </td>
