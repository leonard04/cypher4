                                                    ...
                                                    <div class="modal-body">
                                                        @foreach($step as $keyStep => $itemStep)
                                                            <div class="form-group row">
                                                                ...
                                                                <div class="col-md-8">
                                                                    <select name="associates[{{$keyStep}}]" class="form-control select2">
                                                                        <option value="">&nbsp;</option>
                                                                        {{--get from users--}}
                                                                        @foreach($users as $user)
                                                                            <option value="{{$user->id}}" {{(isset($associates[$keyStep]) && $associates[$keyStep]->id_user == $user->id) ? "SELECTED" : ""}}>{{$user->name}}</option>
                                                                        @endforeach
                                                                    </select>
                                                                </div>
                                                            </div>
                                    ...
                                    @foreach($step as $keyStep => $itemStep)
                                        <div class="row">
                                            ...
                                            <label for="" class="col-md-8">
                                                ...
                                                @else
                                                    <span class="label label-inline {{($associates[$keyStep]->id_user != null) ? "label-info" : ""}}">{{($associates[$keyStep]->id_user != null) ? $user_name[$associates[$keyStep]->id_user] : "N/A"}}</span>
                                                @endif
                                            </label>
                                        </div>
                                        <hr>
                                                ...
                                                @foreach($step as $keyStep => $itemStep)
                                                    @if(!isset($itemStep['isMeeting']))
                                                        @if($associates[$keyStep]->file_draft == null)
                                                            Waiting for {{$itemStep['title']}} draft
                                                            @break
                                                        @elseif($associates[$keyStep]->resi == null)
                                                            Waiting for {{$itemStep['title']}} to be sent
                                                            @break
                                                        @elseif($associates[$keyStep]->file == null)
                                                            Waiting for file {{$itemStep['title']}} signed
                                                            @break
                                                        @endif
                                                    @else
                                                        @if($associates[$keyStep]->id_meeting_internal == null)
                                                            Waiting for {{$itemStep['title']}} internal date
                                                            @break
                                                        @elseif($associates[$keyStep]->file_draft == null)
                                                            Waiting for {{$itemStep['title']}} internal MOM file
                                                            @break
                                                        @elseif($associates[$keyStep]->id_meeting_eksternal == null)
                                                            Waiting for {{$itemStep['title']}} eksternal date
                                                            @break
                                                        @elseif($associates[$keyStep]->file == null)
                                                            Waiting for {{$itemStep['title']}} eksternal MOM file
                                                            @break
                                                        @endif
                                                    @endif
                                                @endforeach
