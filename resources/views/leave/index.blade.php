                ...
                <tbody>
                    @foreach($leave as $key => $value)
                        <tr>
                            ...
                            <td>{{date_diff(date_create($value->awal), date_create($value->akhir))->format('%a')}}</td>
                            ...
                            <td>@if(empty($value->hrd_by))
                                    @if(empty($value->div_by))
                                        waiting
                                    @else
                                        <button type="button" id="btnHrd_{{$key}}" onclick="button_approve({{$value->c_id}}, 'hrd')" class="btn btn-primary btn-xs"> Approve</button>
                                    @endif
                                @else
                                    Approved at {{$value->hrd_date}}
                                @endif</td>
                            <td><button type="button" id="btnDel_{{$key}}" onclick="button_delete({{$value->c_id}})" class="btn btn-danger btn-xs btn-icon"><i class="fa fa-trash"></i></button></td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
