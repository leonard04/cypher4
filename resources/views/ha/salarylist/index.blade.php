@if(!(session()->has('seckey_sallist')) || (session()->has('seckey_sallist') < 10))
    <script>window.location = "{{route('salarylist.needsec')}}";</script>
@endif
                ...
                @foreach($types as $type)
                    ...
                    <div class="tab-pane fade show @if($type->id == 1) active @endif" id="list{{$type->id}}" role="tabpanel" aria-labelledby="home-tab">
                        <form action="{{route('salarylist.save')}}" method="POST">
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <table class="table table-responsive table-bordered font-size-sm table-responsive-xl display" style="margin-top: 13px">
                                        <thead>
                                        <tr>
                                        ...
                                        @foreach($employees as $key => $val)
                                            @if($val->emp_type == $type->id)
                                                <tr>
                                                    ...
                                                    <td class="text-center" width="20%">
                                                        <input type="number" name="YEARLY[{{$val->id}}]" class="form-control" value="{{$val->yearly_bonus}}">
                                                    </td>
                                                    <td class="text-center" width="20%">
                                                        <input type="number" name="FX_YEARLY[{{$val->id}}]" class="form-control" value="{{$val->fx_yearly_bonus}}">
                                                    </td>
                                                    <td class="text-center" width="20%">
                                                        <input type="number" name="THR[{{$val->id}}]" class="form-control" value="{{$val->thr}}">
                                                    </td>
