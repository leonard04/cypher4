        ...
        <div class="card-body">
            <ul class="nav nav-tabs nav-tabs-line mb-5">
                ...
                <li class="nav-item dropdown">
                    ...
                    <div class="dropdown-menu">
                        <a class="dropdown-item" data-toggle="tab" href="#po-out">PO Oustanding</a>
                        <a class="dropdown-item" data-toggle="tab" href="#po-all">All PO</a>
                        <a class="dropdown-item" data-toggle="tab" href="#wo-out">WO Oustanding</a>
                                        ...
                                        @foreach($po_data as $key => $item)
                                            @if($item->po_type == null || $item->po_type == "")
                                                <tr>
                                                    ...
                                                    <td align="center">
                                                        <button type="button" class="btn btn-xs btn-icon btn-primary" onclick="button_edit_type('{{$key}}', 'po')"><i class="fa fa-edit"></i></button>
                                                    </td>
                                                </tr>
                                            @endif
                                        @endforeach
                                        ...
                                        @foreach($wo_data as $key => $item)
                                            @if($item->wo_type == null || $item->wo_type == "")
                                                <tr>
                                                    ...
                                                    <td align="center">
                                                        <button type="button" class="btn btn-xs btn-icon btn-primary" onclick="button_edit_type('{{$key}}', 'wo')"><i class="fa fa-edit"></i></button>
                                                    </td>
                                                </tr>
                                            @endif
                                        @endforeach
...
@section('custom_script')
    <script>
        ...
        function button_edit_type(x, y){
            ...
            if (y === "po"){
                var json_type = "{{json_encode($po_data)}}".replaceAll("&quot;", "\"")
                var iType = "po_type"
            } else {
                var json_type = "{{json_encode($wo_data)}}".replaceAll("&quot;", "\"")
                var iType = "wo_type"
            }

            var data = JSON.parse(json_type)
            console.log(data)
            $.ajax({
                url: "{{route('ha.powotypes.getTypes')}}/" + y,
                type: "get",
                dataType: "json",
                cache: "false",
                success: function(response){
                    ...
                    $("#type-data").val(y)
                    $("#id-data").val(data[x].id)
                    $("#type-update").val(data[x][iType]).trigger("change")
                }
            })
        }
