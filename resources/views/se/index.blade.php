    ...
    <div class="card card-custom gutter-b">
        <div class="card-header">
            <div class="card-title">
                <h3>List Service Evaluation</h3><br>
        ...
        <div class="card-body">
            <div class="row mb-5 mt-5">
                <div class="col-md-12">
                    <img src="{{asset('media/se.png')}}" style="width: 65%">
                </div>
            </div>
            <ul class="nav nav-tabs" id="myTab" role="tablist">
                <li class="nav-item">
                    <a class="nav-link active" id="home-tab" data-toggle="tab" href="#all">
                        ...
                        <span class="nav-text">SE Waiting</span>
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" id="profile-tab" data-toggle="tab" href="#sales" aria-controls="profile">
                        ...
                        <span class="nav-text">SE Bank</span>
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" id="profile-tab" data-toggle="tab" href="#cost" aria-controls="profile">
                        ...
                        <span class="nav-text">SE Rejected</span>
                    </a>
                </li>
            </ul>
                    ...
                    <div id="kt_datatable_wrapper" class="dataTables_wrapper dt-bootstrap4 no-footer">
                        <table class="table table-bordered table-hover display font-size-sm" style="margin-top: 13px !important; width: 100%;">
                            ...
                            <tr>
                                ...
                                <th class="text-left">RFQ Date</th>
                                <th class="text-left">SE#</th>
                            ...
                            @foreach($sr as $i => $item)
                                @if($item->rfq_approved_by != null && $item->se_approved_by == null && $item->se_rejected_by == null)
                                    <tr>
                                        ...
                                        <td align="center">{{$item->rfq_so_num}}</td>
                                        <td align="center">{{date('d F Y', strtotime($item->rfq_so_date))}}</td>
                                        <td align="center">
                                            <a href="{{URL::route('se.view', $item->id)}}" class="text-hover-danger">{{$item->se_num}}</a>
                                        </td>
                                        ...
                                        <td align="center">
                                            @if($item->se_input_by == null)
                                                <a href="{{URL::route('se.appr', $item->id)}}" class="text-hover-danger">waiting <i class="fa fa-clock"></i></a>
                                            @else
                                                inputed at {{date('Y-m-d', strtotime($item->se_input_at))}}
                                            @endif
                                        </td>
                                        <td align="center">
                            ...
                            @foreach($sr as $i => $item)
                                @if($item->se_approved_by != null)
                                    <tr>
                                        ...
                                        <td align="center">{{$view_company[$item->company_id]->tag}}</td>
                                        <td align="center">{{count($det[$item->id])}}</td>
                                        <td align="center">{{date('d F Y', strtotime($item->se_date))}}</td>
                                        <td align="center">
                                            @if($item->se_approved_by == null)
                                                @if($item->act_by != null)
                                                @else
                        ...
                        <table class="table table-bordered table-hover display font-size-sm" style="margin-top: 13px !important; width: 100%;">
                            ...
                            @foreach($sr as $i => $item)
                                @if($item->se_rejected_by != null)
                                    <tr>
                                        <td align="center">{{$i3 + 1}}<?php $i3++ ?></td>
                                        <td align="center">{{$item->rfq_so_num}}</td>
                                        <td align="center">{{date('d F Y', strtotime($item->rfq_so_date))}}</td>
                                        <td align="center">
                                            <a href="{{URL::route('se.view', $item->id)}}" class="text-hover-danger">{{$item->se_num}}</a>
